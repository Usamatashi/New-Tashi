...
const ScanRow = memo(({ item, showLine }: { item: Scan; showLine: boolean }) => {
...
export default function HistoryScreen() {
  ...
  const { data: scans = [], isLoading, isRefetching, refetch } = useQuery<Scan[]>({
    queryKey: ["scans"],
    queryFn: async () => {
  ...
  const totalPoints = useMemo(
    () => scans.reduce((sum, s) => sum + s.pointsEarned, 0),
    [scans],
  );
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <ScanRow item={item} showLine={index < scans.length - 1} />
          )}
        />
      )}
    </View>
