...
const ACTIONS = [
  ...
  },
  {
    ...
    decoration: "#2DD4BF",
    countSummaryKey: "adsCount" as const,
  ...
  },
  {
    ...
    decoration: "#FBBF24",
    countSummaryKey: "tickerCount" as const,
...
export default function AdminDashboard() {
  ...
  const { data: summary, isLoading: loadingCounts } = useQuery({
    queryKey: queryKeys.adminDashboard,
    queryFn: () => apiFetch<DashboardSummary>("/admin-dashboard/summary"),
  });

  const pendingClaims = summary?.pendingClaims ?? 0;
  const pendingOrders = summary?.pendingOrders ?? 0;
  const totalOutstanding = summary?.totalOutstanding ?? null;
      ...
      >
        ...
        <View style={styles.cards}>
          {visibleActions.map((action) => {
            const hasCount = action.countSummaryKey !== null;
            const count = hasCount && summary ? summary[action.countSummaryKey!] : null;
