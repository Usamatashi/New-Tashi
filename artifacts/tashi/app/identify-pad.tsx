...
type Candidate = ProductMatch & { score: number };
...
export default function IdentifyPadScreen() {
  ...
  const identifyPad = async (base64: string) => {
    ...
    setResult(null);
    try {
      setResult(await apiFetch<MatchResult>("/products/identify", {
        method: "POST",
        body: JSON.stringify({ photoBase64: base64 }),
      }));
    } catch (err) {
      const msg = apiErrorMessage(err, "Network error. Please try again.");
    ...
    return (
      <View style={[styles.container, { paddingTop: topPad }]}>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 40 }}>
          <Image source={{ uri: capturedPhoto }} style={styles.capturedImage} contentFit="contain" />
          {identifying && (
            <View style={styles.analyzingBox}>
              <ActivityIndicator size="large" color={Colors.primary} />
              <Text style={styles.analyzingText}>Analyzing pad shape with AI...</Text>
  ...
  return (
    <View style={styles.candidateRow}>
      <View style={styles.candidateRank}>
        <Text style={styles.candidateRankText}>{rank}</Text>
      </View>
      {candidate.imageUrl ? (
        <Image source={{ uri: candidate.imageUrl }} style={styles.candidateImage} contentFit="contain" cachePolicy="memory-disk" />
      ) : (
      ...
      <View style={styles.candidateInfo}>
        ...
        {candidate.productNumber ? (
          <Text style={styles.candidateNumber}>#{candidate.productNumber}</Text>
        ) : null}
        {candidate.vehicleManufacturer ? (
          <Text style={styles.candidateMfr} numberOfLines={1}>
            {candidate.vehicleManufacturer}
          </Text>
        ) : null}
      </View>
      <View style={styles.candidateRight}>
        {candidate.salesPrice > 0 ? (
          <Text style={styles.candidatePrice}>Rs. {candidate.salesPrice.toLocaleString()}</Text>
        ) : null}
        <Text style={styles.candidateScore}>{(candidate.score * 100).toFixed(1)}%</Text>
      </View>
    </View>
  );
}
...
function ResultCard({ result, onReset }: { result: MatchResult; onReset: () => void }) {
  ...
  const matchedId = result.product?.id ?? null;
  const otherCandidates = candidates.filter((c) => c.id !== matchedId);
    ...
    return (
      <View style={styles.resultCard}>
        ...
        <Text style={styles.noMatchTitle}>
          {candidates.length > 0 ? "Possible Matches" : "No Match Found"}
        </Text>
        ...
        {candidates.length > 0 ? (
          <View style={styles.candidateList}>
            <Text style={styles.candidateListTitle}>Top {candidates.length} candidates</Text>
            {candidates.map((c, i) => (
  ...
  return (
    <View style={styles.resultCard}>
      ...
      {p.imageUrl ? (
        <Image source={{ uri: p.imageUrl }} style={styles.matchImage} contentFit="contain" cachePolicy="memory-disk" />
      ) : (
      ...
      {otherCandidates.length > 0 ? (
        <View style={styles.candidateList}>
          <Text style={styles.candidateListTitle}>Other possible matches</Text>
...
const styles = StyleSheet.create({
  ...
  candidateRankText: { color: Colors.white, fontSize: 12, fontFamily: "Inter_700Bold" },
  candidateImage: { width: 44, height: 44, borderRadius: 8, backgroundColor: "#fff" },
  candidateImagePlaceholder: { alignItems: "center", justifyContent: "center" },
  candidateInfo: { flex: 1, gap: 2 },
  candidateName: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: Colors.text },
  candidateNumber: { fontSize: 11, fontFamily: "Inter_700Bold", color: Colors.primary },
  candidateMfr: { fontSize: 11, fontFamily: "Inter_400Regular", color: Colors.textSecondary },
  candidateRight: { alignItems: "flex-end", gap: 2 },
  candidatePrice: { fontSize: 12, fontFamily: "Inter_700Bold", color: Colors.text },
  candidateScore: {
