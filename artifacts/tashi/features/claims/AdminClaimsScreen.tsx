...
export default function AdminClaimsScreen() {
  ...
  useFocusEffect(useCallback(() => { refetchClaims(); }, [refetchClaims]));
  ...
  const fetchClaimScans = async (claim: Claim) => {
    ...
    try {
      setClaimScans(await apiFetch<ClaimScan[]>(`/claims/${claim.id}/scans`));
    } catch {
      setClaimScans([]);
    } finally {
  ...
  const handleQrScanned = async ({ data }: { data: string }) => {
    ...
    try {
      const result = await apiFetch<{
        scanId: number;
        verifiedPoints: number;
      }>(`/claims/${verifyingClaim.id}/verify-qr`, {
        method: "POST",
      ...
      setClaimScans((prev) =>
        prev.map((s) => (s.id === result.scanId ? { ...s, adminVerified: true } : s)),
      );
      setVerifyingClaim((prev) =>
        prev ? { ...prev, verifiedPoints: result.verifiedPoints, verifiedScans: prev.verifiedScans + 1 } : prev,
      );
      queryClient.setQueryData<Claim[]>(queryKeys.claims, (prev) =>
        (prev ?? []).map((c) =>
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const result = await apiFetch<{ verifiedPoints: number }>(
                `/claims/${verifyingClaim.id}/mark-missing`,
                { method: "POST", body: JSON.stringify({ scanId: scan.id }) },
              );
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              await apiFetch(`/claims/${verifyingClaim.id}`, { method: "PATCH" });
              Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => refetchClaims()} style={styles.refreshBtn}>
