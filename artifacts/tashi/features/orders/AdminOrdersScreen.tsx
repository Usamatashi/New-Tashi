...
type OrderSourceTab = "website" | "wholesale";
type StatusFilter = "pending" | "dispatched" | "cancelled";
...
const SOURCE_TABS: { key: OrderSourceTab; label: string; icon: React.ComponentProps<typeof Feather>["name"] }[] = [
...
function matchesWholesaleFilter(status: string, filter: StatusFilter): boolean {
  const s = status.toLowerCase();
  if (filter === "pending") return s === "pending" || s === "confirmed";
  if (filter === "dispatched") return s === "dispatched";
  return s === "cancelled";
}

function matchesWebsiteFilter(status: string, filter: StatusFilter): boolean {
  const s = status.toLowerCase();
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={editStyles.root}>
        ...
        <KeyboardAwareScrollViewCompat contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
...
function BillModal({
  ...
  const billDiscPct = order.billDiscountPercent ?? 0;
  const subtotal = order.subtotal ?? items.reduce((s, i) => s + (i.discountedValue ?? i.totalValue), 0);
  const billDiscAmt = order.billDiscountAmount ?? 0;
  const grandTotal = order.finalAmount ?? subtotal;
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={billStyles.root}>
        ...
        <KeyboardAwareScrollViewCompat contentContainerStyle={{ padding: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
...
function WebsiteOrderCard({
  ...
  const status = order.status.toLowerCase() as WebsiteOrder["status"];
  const statusKey = status in STATUS_COLOR ? status : "pending";

  return (
    <View style={styles.card}>
      <TouchableOpacity
        style={styles.cardHeader}
        onPress={() => { Haptics.selectionAsync(); setExpanded((p) => !p); }}
        activeOpacity={0.7}
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.cardDate}>{order.customer.phone || "—"} · {date}</Text>
          <Text style={[styles.cardDate, { marginTop: 2 }]}>
            {order.delivery.city || "—"}
          </Text>
        </View>
        <View style={{ alignItems: "flex-end", gap: 4 }}>
          <View style={[styles.statusPill, { backgroundColor: STATUS_BG[statusKey] ?? STATUS_BG.pending }]}>
            <Text style={[styles.statusText, { color: STATUS_COLOR[statusKey] ?? STATUS_COLOR.pending }]}>
              {status.toUpperCase()}
            </Text>
          </View>
          <Text style={styles.cardTotalInline}>Rs. {Math.round(order.total).toLocaleString()}</Text>
        </View>
        <Feather
      ...
      {expanded && (
        <>
          <View style={{ paddingHorizontal: 16, paddingBottom: 8 }}>
            <Text style={styles.webMeta}>Order #{shortId} · {order.items.length} item{order.items.length === 1 ? "" : "s"}</Text>
            {order.delivery.address ? (
              <Text style={styles.webAddress} numberOfLines={2}>{order.delivery.address}</Text>
            ) : null}
            {order.payment.method ? (
              <Text style={styles.webMeta}>Payment: {order.payment.method.toUpperCase()}</Text>
            ) : null}
          </View>
          ...
          <View style={styles.table}>
            {order.items.map((item, idx) => (
              <View
                ...
                style={[styles.tableRow, { paddingVertical: 8, borderBottomWidth: idx < order.items.length - 1 ? 1 : 0, borderBottomColor: "#F0EDE8" }]}
              >
                <Text style={[styles.colVal, { flex: 1 }]} numberOfLines={2}>{item.productName}</Text>
                <Text style={[styles.colVal, styles.colCenter, { width: 36 }]}>×{item.quantity}</Text>
                <Text style={[styles.colVal, styles.colRight, styles.colTotal, { width: 72 }]}>
                  {Math.round(item.lineTotal ?? item.unitPrice * item.quantity).toLocaleString()}
                </Text>
              </View>
            ))}
            <View style={styles.totalFooter}>
              <Text style={styles.totalFooterLabel}>Total</Text>
              <Text style={styles.totalFooterValue}>Rs. {Math.round(order.total).toLocaleString()}</Text>
            </View>
          </View>
...
function OrderCard({
  ...
  const items = order.items ?? [];
  const hasItemDiscounts = items.some(i => (i.discountPercent ?? 0) > 0);
  const orderBillPct = order.billDiscountPercent ?? 0;
  const orderSubtotal = order.subtotal ?? items.reduce((s, i) => s + (i.discountedValue ?? i.totalValue), 0);
  const orderBillDiscAmt = order.billDiscountAmount ?? 0;
  const grandTotal = order.finalAmount ?? orderSubtotal;
          ...
          <View style={styles.table}>
            ...
            {orderBillPct > 0 && (
              <View style={[styles.tableRow, { paddingVertical: 6 }]}>
                <Text style={[styles.colHead, { flex: 1, color: "#16a34a" }]}>Bill Discount ({orderBillPct}%)</Text>
                <Text style={[styles.colVal, styles.colRight, { flex: 1, color: "#16a34a" }]}>−Rs. {orderBillDiscAmt.toLocaleString()}</Text>
              </View>
            )}
...
export default function AdminOrdersScreen() {
  ...
  const qc = useQueryClient();
  const [sourceTab, setSourceTab] = useState<OrderSourceTab>("website");
  const [statusFilter, setStatusFilter] = useState<StatusFilter>("pending");
  const [selectedDate, setSelectedDate] = useState<string | null>(null);
  const [calendarMonth, setCalendarMonth] = useState(() => new Date());
  const [showCalendar, setShowCalendar] = useState(false);
  const [updatingId, setUpdatingId] = useState<number | null>(null);
  const [updatingWebId, setUpdatingWebId] = useState<string | null>(null);
  ...
  const { data: websiteData, isLoading: webLoading, refetch: refetchWeb, isRefetching: webRefetching } = useQuery<{ orders: WebsiteOrder[] }>({
    queryKey: ["admin-website-orders"],
    queryFn: () => apiFetch("/website-orders"),
  });
  const websiteOrders = websiteData?.orders ?? [];

  const { data: orders = [], isLoading: wholesaleLoading, refetch: refetchWholesale, isRefetching: wholesaleRefetching } = useQuery<Order[]>({
  ...
  const updateWebsiteStatus = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      apiFetch(`/website-orders/${encodeURIComponent(id)}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status }),
      }),
    onMutate: ({ id }) => setUpdatingWebId(id),
    onSettled: () => {
      setUpdatingWebId(null);
  ...
  const handleCancelWebsite = useCallback((id: string) => {
    Alert.alert("Cancel Order", "Are you sure you want to cancel this website order?", [
      { text: "No", style: "cancel" },
      {
        text: "Yes, Cancel",
        style: "destructive",
        onPress: () => updateWebsiteStatus.mutate({ id, status: "cancelled" }),
      },
    ]);
  }, [updateWebsiteStatus]);

  const matchesDate = (createdAt: string) =>
    !selectedDate || toDateKey(new Date(createdAt)) === selectedDate;

  const filteredWebsite = websiteOrders
    .filter((o) => matchesWebsiteFilter(o.status, statusFilter))
    .filter((o) => matchesDate(o.createdAt));

  const filteredWholesale = orders
    .filter((o) => matchesWholesaleFilter(o.status, statusFilter))
    .filter((o) => matchesDate(o.createdAt));

  const isLoading = sourceTab === "website" ? webLoading : wholesaleLoading;
  const isRefetching = sourceTab === "website" ? webRefetching : wholesaleRefetching;
  const refetch = sourceTab === "website" ? refetchWeb : refetchWholesale;
  const listEmpty = sourceTab === "website" ? filteredWebsite.length === 0 : filteredWholesale.length === 0;
  ...
  return (
    <View style={[styles.root, { paddingTop: topPad }]}>
      <View style={styles.header}>
        ...
        <TouchableOpacity
          style={[styles.calendarIconBtn, selectedDate && styles.calendarIconBtnActive]}
          onPress={() => setShowCalendar(true)}
          activeOpacity={0.8}
        >
          <Feather name="calendar" size={18} color={selectedDate ? "#fff" : Colors.adminAccent} />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.sourceTabRow}>
        {SOURCE_TABS.map((tab) => {
          ...
          return (
            <TouchableOpacity
              key={tab.key}
              style={[styles.sourceTabBtn, active && styles.sourceTabBtnActive]}
              onPress={() => { Haptics.selectionAsync(); setSourceTab(tab.key); setStatusFilter("pending"); }}
              activeOpacity={0.85}
            >
              <Feather name={tab.icon} size={14} color={active ? "#fff" : Colors.textSecondary} />
              <Text style={[styles.sourceTabText, active && styles.sourceTabTextActive]}>{tab.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
      ...
      <View style={styles.statusSegmentRow}>
        {STATUS_FILTERS.map((f) => {
          ...
          return (
            <TouchableOpacity
              key={f.key}
              style={[styles.statusSegment, active && styles.statusSegmentActive]}
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.emptyTitle}>No {statusFilter} orders</Text>
          <Text style={styles.emptyText}>
            {selectedDate
              ? "Try another date or tap the calendar icon to clear the filter"
              : sourceTab === "website"
                ? "New website orders will appear here."
                : "Wholesale orders from the mobile app will appear here."}
          </Text>
        </View>
      ) : sourceTab === "website" ? (
        <FlatList
          data={filteredWebsite}
          keyExtractor={(item) => item.id}
          renderItem={({ item }) => (
            <WebsiteOrderCard
              order={item}
              onCancel={handleCancelWebsite}
              onDispatch={(id) => updateWebsiteStatus.mutate({ id, status: "dispatched" })}
              isUpdating={updatingWebId === item.id}
            />
          )}
