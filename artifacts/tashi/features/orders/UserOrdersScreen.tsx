...
function SalesmanEditOrderModal({
    ...
    if (visible && order && products.length > 0) {
      const initial: CartItem[] = order.items
        .map(item => {
          ...
          return p ? { product: p, quantity: item.quantity, discountPercent: item.discountPercent ?? 0 } : null;
        })
  ...
  const addToCart = () => {
    ...
    setCart(prev => {
      ...
      if (idx >= 0) {
        const updated = [...prev];
        updated[idx] = { product: selectedProduct, quantity: qty, discountPercent: 0 };
        return updated;
      }
      return [...prev, { product: selectedProduct, quantity: qty, discountPercent: 0 }];
    });
    setSelectedProduct(null);
    setQuantity("1");
    setTab("cart");
  ...
  const handleSave = () => {
    ...
    setSaveError("");
    saveItemsMutation.mutate(cart.map(c => ({ productId: c.product.id, quantity: c.quantity, discountPercent: c.discountPercent })));
  };
          ...
          {items.map((item, idx) => {
            ...
            return (
              <View key={idx} style={[styles.tableRow, { paddingVertical: 8, borderBottomWidth: idx < items.length - 1 ? 1 : 0, borderBottomColor: "#F0EDE8" }]}>
                ...
                <Text style={[styles.colVal, styles.colRight, { flex: 2, color: discPct > 0 ? "#10B981" : undefined, fontWeight: discPct > 0 ? "700" : "400" }]}>
...
export default function OrdersScreen() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => {
      ...
      return name.includes(q) || num.includes(q) || idStr.includes(q);
    });
        ...
        >
          ...
          {step === 2 && (
            <View>
              ...
              {cartItems.length > 0 && (
                <View style={[styles.rowBtns, { marginTop: 16, marginBottom: 8 }]}>
              ...
              {!selectedProduct ? (
                <>
                  ...
                  <View style={styles.emptyState}>
                    ...
                    <Text style={styles.emptyText}>Try a different name or number</Text>
                  </View>
                ) : !Array.isArray(products) ? (
                  <View style={styles.center}><Text>Error loading products</Text></View>
                ) : (
                  CATEGORY_ORDER.map(cat => {
                    const catProducts = filteredProducts.filter(p => (p.category ?? "other") === cat);
                    if (catProducts.length === 0) return null;
                    const meta = CATEGORY_META[cat];
                    return (
                      <View key={cat}>
                            ...
                            >
                              ...
                              <View style={{ flex: 1 }}>
                                ...
                                <Text style={styles.selectCardSub}>
                                  {v.productNumber ? `#${v.productNumber} · ` : ""}
                                  Rs. {v.salesPrice.toLocaleString()}/unit
                                </Text>
                              </View>
                              {alreadyAdded && (
              ...
              {cartItems.length === 0 && (
                <TouchableOpacity style={[styles.secondaryBtn, { marginTop: 24 }]} onPress={() => setStep(1)}>
                  <Feather name="arrow-left" size={16} color={Colors.primary} />
                  <Text style={styles.secondaryBtnText}>Back</Text>
                </TouchableOpacity>
              )}
