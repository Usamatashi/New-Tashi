...
export default function CreateAccountScreen() {
  ...
  const { data: regions = [], isFetching: regionsLoading, refetch: refetchRegions } = useQuery({
    queryKey: queryKeys.adminRegions,
  ...
  const openEdit = async (user: User) => {
    ...
    if (user.role === "admin" && currentUser?.role === "super_admin") {
      try {
        const data = await apiFetch<typeof DEFAULT_ACCESS>(`/admin-user-settings/${user.id}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await apiFetch<User & { id?: number }>(path, {
        method,
      ...
      if (role === "admin" && currentUser?.role === "super_admin") {
        ...
        if (userId) {
          await apiFetch(`/admin-user-settings/${userId}`, {
            method: "PUT",
      ...
      setErrorMsg(apiErrorMessage(err, "Network error. Please try again."));
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await apiFetch(`/users/${confirmUser.id}`, { method: "DELETE" });
      setConfirmUser(null);
      queryClient.invalidateQueries({ queryKey: queryKeys.adminUsers });
    } catch {
    } finally {
            ...
            {contactsLoading ? (
              ...
              <FlatList
                ...
                keyExtractor={(item, index) =>
                  (item as { id?: string }).id ?? item.name ?? `contact-${index}`
                }
