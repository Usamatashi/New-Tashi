...
- **Cloud Run (recommended):** `GCP_PROJECT_ID=your-id bash scripts/deploy-cloud-run.sh`
- **App Engine:** Build locally then `gcloud app deploy artifacts/api-server/app.yaml`
- **CI/CD:** Connect repo to Cloud Build — `cloudbuild.yaml` is at the root

Key files:
- `Dockerfile` — multi-stage Docker build for Cloud Run
- `.dockerignore` — excludes non-API files from build context
- `cloudbuild.yaml` — automates build + push + deploy
- `artifacts/api-server/app.yaml` — App Engine config
- `scripts/deploy-cloud-run.sh` — one-command deploy script
...
| `PORT` | Runtime | Assigned by Replit / Cloud Run (auto-set to 8080) |
